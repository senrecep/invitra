    ...
    <div className="bg-white rounded-2xl shadow-[0_1px_6px_rgba(0,0,0,0.06)] border border-stone-200 overflow-hidden">
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          {/* Event info banner */}
          {settings?.eventName && (
            <div className="bg-gradient-to-r from-rose-50 to-stone-50 rounded-xl px-4 py-3 border border-rose-100 space-y-1">
              <p className="text-base font-bold text-slate-800 leading-tight">{settings.eventName}</p>
              ...
              {(settings.eventDate || settings.eventTime) && (
                <div className="flex items-center gap-2 pt-0.5">
                  ...
                  {settings.eventDate && settings.eventTime && <span className="text-stone-300">·</span>}
                  {settings.eventTime && (
                    <span className="text-xs font-semibold text-rose-600">{settings.eventTime}</span>
                  )}
                </div>
              )}
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-stone-100 rounded-xl p-3 text-center">
                  <p className="text-2xl font-bold text-slate-700">{stats.totalGuests}</p>
                  <p className="text-xs text-slate-500 mt-0.5 leading-tight">{t("totalGuests")}</p>
                </div>
              ...
              {stats.byGroup.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide">{t("byGroup")}</p>
                  <div className="space-y-1.5">
                    {stats.byGroup.map((g) => (
                      <div key={g.id} className="flex items-center gap-2 bg-stone-50 rounded-xl px-3 py-2">
                        ...
                        <div className="flex gap-3 text-xs text-slate-500 shrink-0">
                          <span className="text-rose-600 font-medium">{g.guestCount} {t("guests")}</span>
              ...
              {stats.byOrganizer.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide">{t("byOrganizer")}</p>
                  <div className="space-y-1.5">
                    {stats.byOrganizer.map((o) => (
                      <div key={o.id} className="flex items-center gap-2 bg-stone-50 rounded-xl px-3 py-2">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-slate-800 truncate">{o.name}</p>
                        </div>
                        <div className="flex gap-3 text-xs text-slate-500 shrink-0">
                          <span className="text-rose-600 font-medium">{o.guestCount} {t("guests")}</span>
              ...
              {stats.byTransportation.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide">{t("byTransportation")}</p>
                  <div className="grid grid-cols-3 gap-2">
